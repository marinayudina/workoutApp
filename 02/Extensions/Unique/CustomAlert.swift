...
//  02
//
//  Created by Марина on 04.04.2023.
//
...
class CustomAlert {
    ...
    private let backgroundView: UIView = {
        ...
        view.alpha = 0 //полностью прозрачная
        return view
    }()
    ...
    private let setTextField = BrownTextField()
    ...
    func presentCustomAlert(viewController: UIViewController,
        ...
        let sportmanImageView = UIImageView(frame: CGRect(x: (alertView.frame.width -
                                                              alertView.frame.height * 0.4) / 2,
                                                          y: 30,
                                                          width: alertView.frame.height * 0.4,
                                                          height: alertView.frame.height * 0.4))
        sportmanImageView.image = UIImage(named: "sportsmanAlert")
        sportmanImageView.contentMode = .scaleAspectFit
        alertView.addSubview(sportmanImageView)
        
        let editingLabel = UILabel(text: "Editing" ,
                                   font: .robotoMedium22(),
        ...
        okButton.addTarget(self, action: #selector(okButtonTapped),
                           for: .touchUpInside)
    ...
    @objc private func okButtonTapped() {
        ...
        guard let setsNumber = setTextField.text,
              let repsNumber = repsTextField.text
        else { return }
        
        buttonAction?(setsNumber, repsNumber)
        
        guard let targetView = mainView
        else { return }
    ...
    private func registerForKeyboardNotification() { //устанавливаем наблюдателя
        NotificationCenter.default.addObserver(self, //когда появится клава
                                               selector: #selector(keyBoardWillShow),
                                               name: UIResponder.keyboardWillShowNotification,
                                               object: nil)
        
        NotificationCenter.default.addObserver(self, //когда спрячется клава
