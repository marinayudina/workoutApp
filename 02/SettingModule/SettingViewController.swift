...
//  02
//
//  Created by Марина on 23.06.2023.
//
...
extension SettingsViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    private func chooseImagePicker(source: UIImagePickerController.SourceType) {
        if UIImagePickerController.isSourceTypeAvailable(source) {
            ...
            imagePicker.allowsEditing = true //доступно редактирование
            imagePicker.sourceType = source
            present(imagePicker,animated: true)
        }
    }
    
    //что сделать как выбрали или изменили фото
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        let image = info[.editedImage] as? UIImage
        addPhotoImageView.image = image
        addPhotoImageView.contentMode = .scaleAspectFit
        dismiss(animated: true)//закроет imagepicker
    }
}
...
extension SettingsViewController: PHPickerViewControllerDelegate {
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        results.forEach { result in
            result.itemProvider.loadObject(ofClass: UIImage.self) { reading, error in
                ...
                DispatchQueue.main.async { //UI обновляется  в главном потоке
    ...
    private func presentPHPicker() {
        ...
        phPickerConfig.selectionLimit = 1//можно выбрать только один объект
